...
for (( i=1; i<=$GKE_VOLUME_QTY; i++ )); do
	...
	#OPTION 1 for persistence: leverage GCE disks can be directly added as volumes to deployments
	# add DISK to deployment template to mount as volume -- for direct DISK in Deployment (without PV-PVC-CLAIM spec)
	# requires that the DEPLOYMENT file linked in env.sh is the one linking directDisks as "gcePersistentDisk:"
	disk_temp="__GCE_DISK_"$i"__"
	sed -i '' "s,$disk_temp,$DISK,g" $DEPLOYMENT_FILE

	#OPTION 2 for persistence: manually control the entire Disk - Persistent Volume - Pers. Vol. Claim cycle. Add Claim to Deployment)
